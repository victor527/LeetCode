...
//At first with a separate function
/*void AddNewWord(string word, vector<string>& wordList, queue<string>& visit)
{
...
{
    queue<string> visit;
    //AddNewWord(beginWord, wordList, visit);
    int distance = 1; // The begin word and the end word also counts
    visit.push(beginWord);
    while(!visit.empty()){
        int n = visit.size();
        for(int i = 0; i < n; i++){
            string word = visit.front();
            //cout << "word herererererere = " << word << endl;
            visit.pop();
            if(word == endWord)
                return distance;
            //AddNewWord(word, wordList, visit);
            for(int i = 0; i < word.length(); i++){
                char letter = word[i];
                for(int j = 0; j < 26; j++){
                    word[i] = 'a' + j;
...
/*int ladderLength(string beginWord, string endWord, vector<string>& wordList){
    unordered_set<string> visit,wlist;
    int dist=1;
...
//Trial 2 with unordered_set for visit Time = 9792.9ms
/*int ladderLength(string beginWord, string endWord, vector<string>& wordList)
{
...
// Trial 3 change the wordlist into unordered_set Time = 83.77ms
int ladderLength(string beginWord, string endWord, vector<string>& wordList)
{
    queue<string> visit;
